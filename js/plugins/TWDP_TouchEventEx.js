...
(function($) {
  ...
  TouchInput._judgeBackGestrue = function(event) {
    for (var i = 0; i < event.changedTouches.length; i++) {
      ...
      if (distance > this.clickDelta && distance / time > this.backGestureSpeedThreshold && x < this._pressed.touch.pressX) {
        return true;
      }
    }
    return false;
  ...
  Window_Selectable.prototype.isTouchedInsideContents = function() {
    ...
    if (Imported.TWDP_BaseWindowEx) {
      ...
      return x >= padding && y >= padding && x < this.width - padding * 2 && y < this.height - padding * 2;
    }
